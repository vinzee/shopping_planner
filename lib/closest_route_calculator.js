...
ClosestRouteCalculator.find = function(lat, lng, radius, product_types, callback){
	...
	if(!_.isEmpty(product_types)){
		radius = radius / 0.000621371; // miles to meters conversion

        Shop.aggregate([
			{$geoNear: { near: { type: "Point", coordinates: [ lng, lat ]}, distanceField: "distance", includeLocs: "coordinates", spherical: true, maxDistance: radius}},
			{$match: { subcategory: { $in: product_types } } },
			{$group: { _id: "$subcategory", coordinates: { $push: '$coordinates' }, ids: { $push: '$_id' }, names: { $push: '$name' }, addresses: { $push: '$address' } } }
		], function(err, data){
			...
			if(!_.isEmpty(diff)){
				...
				console.log('$$$ algo input : ' , JSON.stringify(data));
...
ClosestRouteCalculator.format_data = function(data){
	_.each(data, function(group){
		...
		for(var i = 0; i < group.coordinates.length; i++)
			group.data.push({coordinate: group.coordinates[i].reverse(), id: group.ids[i], name: group.names[i], address: group.addresses[i]});
